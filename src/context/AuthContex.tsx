...
import { Usuario, LoginResponse, LoginData, RegisterData,RenameResponse, RenameData } from '../interfaces/appInterfaces';
...
type AuthContextProps = {
    ...
    rename: (renameData: RenameData) =>Promise<void>
...
export const AuthProvider = ({ children }: any)=> {
    ...
    const signIn= async( {correo,password}:LoginData) => {
        try{
            ...
            dispatch({type:'addError',
                payload:error.response.data.msg||error.response.data.errors[0].msg||'Inicio de sesión invalido'
            })
        }
    };
    ...
    const rename = async ({ uid,nombre}: RenameData) => {
        try {
            const { data } = await mathApi.put<RenameResponse>(`/usuarios/${uid}`, {
                nombre,
            });
            dispatch({
                type: 'rename',
